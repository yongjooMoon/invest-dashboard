...
def generate_excel_data(api_key, district_code, district_name, target_dong, start_date, end_date, apt_filters):
    ...
    # 💡 [핵심 패치] 엑셀 병합 및 Min/Max/Avg 요약 로직 100% 원복
    ...
    for col_idx in range(1, 14):
        ...
        cell.font = font_header; cell.fill = fill_header; cell.alignment = align_center; cell.border = cell_border
    ...
    for key in sorted_keys:
        ...
        if cls_type == "주상복합" and "주상복합" not in display_apt_name: display_apt_name += "(주상복합)"
        elif cls_type == "도시형생활주택" and "도시형" not in display_apt_name: display_apt_name += "(도시형생활주택)"
        ...
        for deal in deals:
            ...
            row_data = [region_text, display_apt_name, str(move_in_text), h_val, supply_m2, exclu_ar, supply_py, exclu_py, ratio, final_deal_amount, supply_price, exclu_price, final_deal_date]
            ws.append(row_data)
            ...
            for col_idx in range(1, 14):
                ...
                elif col_idx == 9: cell.number_format = '0%'
                elif col_idx == 4 and isinstance(h_val, int): cell.number_format = '#,##0'
                elif col_idx in [10, 11, 12]: cell.number_format = '#,##0'
        ...
        for col_idx in range(1, 14):
            ...
            if col_idx >= 3:
                cell.fill = fill_summary; cell.font = font_summary
            else:
                cell.font = font_body
    ...
    ws.column_dimensions['C'].width = 16; ws.column_dimensions['J'].width = 25
    ws.column_dimensions['K'].width = 18; ws.column_dimensions['L'].width = 18
