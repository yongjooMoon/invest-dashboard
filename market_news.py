...
def news_detail_dialog():
    ...
    # 🌟 [링크 분리 핵심 로직] summary의 마지막 부분을 띄어쓰기로 잘라서 http로 시작하는지 확인
    raw_summary = news.get('summary', '')
    parts = raw_summary.rsplit(' ', 1)
    
    news_url = ""
    summary_core = raw_summary
    
    if len(parts) == 2 and parts[1].startswith('http'):
        summary_core = parts[0]
        news_url = parts[1]

    # 이스케이프 처리 후 <br> 변환
    safe_summary_core = html.escape(summary_core)
    summary_text = re.sub(r'(\d\.)', r'<br><br>\1', safe_summary_core)
    ...
    # URL이 존재하면 원문 기사 링크 버튼 추가
    link_html = ""
    if news_url:
        link_html = f"<div style='margin-top:25px;'><a href='{news_url}' target='_blank' style='display:inline-block; padding:10px 18px; background-color:rgba(56,189,248,0.15); color:#38BDF8; border:1px solid rgba(56,189,248,0.3); border-radius:8px; text-decoration:none; font-weight:700; font-size:14px; transition:all 0.2s;'>🔗 원문 기사 보러가기</a></div>"
...
def run_news_page(supabase):
    ...
    with col_h2:
        if today_major_news:
            ...
            st.button("◀", disabled=(st.session_state.carousel_idx <= 0), width="stretch", on_click=carousel_prev)
            st.markdown("</div>", unsafe_allow_html=True)
    with col_h3:
        if today_major_news:
            st.markdown("<div class='slider-btn'>", unsafe_allow_html=True)
            st.button("▶", disabled=(st.session_state.carousel_idx >= max_idx), width="stretch", on_click=carousel_next, args=(max_idx,))
            st.markdown("</div>", unsafe_allow_html=True)
