...
def render_exit_risk_content(h, supabase):
    ...
    # 🛡️ XSS 방어: 종목명 이스케이프
    safe_name = html.escape(h.get('name', ''))
...
def render_detailed_report_content(sel, df_price=None, fund=None, factor_score=None, gates=None):
    ...
    # 🛡️ XSS 방어: 렌더링되는 모든 문자열 이스케이프
    safe_sector = html.escape(sel.get('sector', ''))
    safe_name = html.escape(sel.get('name', ''))
    safe_symbol = html.escape(sel.get('symbol', ''))
    safe_region = html.escape(sel.get('region', 'KR'))

    sector_str = f" &nbsp;|&nbsp; <span style='color:#F8B12A;'>{safe_sector}</span>" if safe_sector and safe_sector != "미분류" else ""

    st.markdown(f"## {safe_name} <span style='font-size:18px; color:#AEC1D4;'>{safe_symbol} &nbsp;|&nbsp; {safe_region} {sector_str}</span>", unsafe_allow_html=True)
    st.markdown(f"""
        ...
        with c_right:
            ...
            if gates:
                for k, g in gates.items():
                    ...
                    st.markdown(f"**{html.escape(g['name'])}** : <span style='color:{color}; font-weight:bold;'>{pass_status}</span> &nbsp;({html.escape(g['reason'])})", unsafe_allow_html=True)
            else:
...
def render_watchlist_grid(items, title, color_code, anchor_id):
    ...
    with st.container():
        ...
        for idx, w in enumerate(items):
            ...
            # 🛡️ XSS 방어
            safe_w_name = html.escape(w.get('name', ''))
            ...
            with c6:
                # 💡 구형 문법(use_container_width)을 최신 문법(width="stretch")으로 교체
        ...
        if holdings:
            ...
            with st.container():
                ...
                for h in holdings:
                    ...
                    # 🛡️ XSS 방어
                    safe_h_name = html.escape(h.get('name', ''))
