...
# ==========================================
# [Layer 1] 유틸리티 및 전역 마스터 주식 풀 (업종 고정식 캐시)
# ==========================================
def parse_num(txt):
...
# 👍 [IP 차단 영구 우회] 국장 포트폴리오 마스터 자산 정보 및 업종 고정 사전
MASTER_STOCK_POOL = {
    "000660": {"name": "SK하이닉스", "sector": "반도체 제조업"},
    "001820": {"name": "삼화콘덴서", "sector": "전기장비 제조업"},
    "006220": {"name": "광전자", "sector": "전자부품 제조업"},
    "001510": {"name": "SK증권", "sector": "증권 / 금융지원업"},
    "003470": {"name": "유안타증권", "sector": "증권 / 금융지원업"},
    "032830": {"name": "삼성생명", "sector": "보험업"},
    "066570": {"name": "LG전자", "sector": "전자부품 제조업"},
    "100790": {"name": "미래에셋벤처투자", "sector": "기타 금융업"},
    "006345": {"name": "대원전선우", "sector": "절연선 및 케이블업"},
    "090360": {"name": "로보스타", "sector": "특수 목적용 기계업"},
    "095610": {"name": "테스", "sector": "반도체 제조 기계업"}
}

# ==========================================
# [Layer 2] 인스티튜셔널 글로벌 매크로 및 수급 크롤러
# ==========================================
...
def calculate_intrinsic_target(row, cache, macro_multiplier, current_usd, df_kospi, df_stock):
    ...
    # 1차 DB 조회 -> 없으면 수동 마스터 캐시 업종 할당
    krx_sector_name = cache.get('krx_sector', MASTER_STOCK_POOL.get(ticker, {}).get('sector', '일반제조업'))
...
# ==========================================
# [Layer 4] 👍 [대표님 이중 캐싱 구조] 1회 로드 및 온디맨드 싱크
# ==========================================
def execute_on_demand_sync(supabase, username, naver_id, naver_secret, force=False):
    ...
    # 👍 [최초 1회 동기화 설계] 캐시에 없는 자산이 존재할 때만 단발성으로 KRX 전체 스캔
    need_krx_call = False
    for row in portfolio_data:
        cached_sec = cache_map.get(row['ticker'], {}).get('krx_sector', '')
        if not cached_sec or "차단" in cached_sec or "미등록" in cached_sec:
            need_krx_call = True
            break
    ...
    for row in portfolio_data:
        ...
        # 1계위: 기구축 DB 캐시 -> 2계위: 거래소 라이브 -> 3계위: 마스터 풀 하드코딩
        db_sector = db_cache.get('krx_sector', '')
        if db_sector and "차단" not in db_sector and "미등록" not in db_sector:
            final_sector = db_sector
        else:
            final_sector = krx_db.get(ticker, MASTER_STOCK_POOL.get(ticker, {}).get('sector', '일반제조업'))
        ...
        if is_expired(db_cache.get('last_fundamental_update'), 604800) or force:
            ...
            if fund_data_memory:
                updated_cache.update({
                    'per': fund_data_memory['per'], 'eps': fund_data_memory['eps'], 'pbr': fund_data_memory['pbr'], 'bps': fund_data_memory['bps'],
    ...
    if stock_cache_batch: supabase.table("stock_cache").upsert(stock_cache_batch).execute()
    if user_portfolio_batch: supabase.table("user_portfolio").upsert(user_portfolio_batch).execute()

# ==========================================
# [Layer 5] 👍 [v17.5 UI 전면 개조] 트레이더 직관형 통합 데스크
# ==========================================
def run_stock_quant_page(supabase, username, naver_id=None, naver_secret=None):
    st.title("🛡️ 스마트 프랍 퀀트 포트폴리오 엔진 v17.5")
    ...
    with st.container(border=True):
        ...
        with m_col2:
            st.metric("시장 기본 PER 멀티플 보정률", f"{int(macro_mult*100)}%", delta="1회 레이지 로딩 캐싱 시스템 작동 중")

    # 👍 [인텔리전스 검색 편입 가동] 대소문자 제거, 공백 제거, 일부 단어 indexOf 매칭
    with st.expander("➕ 신규 자산 즉시 편입 (유연한 검색 등록 가동)", expanded=False):
        st.markdown("**초성, 기업명 일부, 띄어쓰기 오염을 무시하는 지능형 파서가 가동됩니다.**")
        user_search_input = st.text_input("종목명 또는 코드 일부를 치고 Enter (예: 하이닉스, sk, 삼화, 000660)", "").strip()
        
        matched_ticker, matched_name = None, None
        if user_search_input:
            clean_query = user_search_input.upper().replace(" ", "")
            for tk, val in MASTER_STOCK_POOL.items():
                clean_pool_name = val['name'].upper().replace(" ", "")
                if clean_query in tk or clean_query in clean_pool_name:
                    matched_ticker = tk
                    matched_name = val['name']
                    break
        
        if matched_ticker and matched_name:
            st.info(f"🔎 매칭 성공: **{matched_name} ({matched_ticker})** 섹터: `{MASTER_STOCK_POOL[matched_ticker]['sector']}`")
            with st.form("quick_add_asset_form"):
                c_in1, c_in2 = st.columns(2)
                with c_in1: add_p = st.number_input("매수 평단가 (₩)", min_value=0, step=100, value=1000)
                with c_in2: add_q = st.number_input("매수 수량 (주)", min_value=1, step=10, value=10)
                if st.form_submit_button("💼 이 종목으로 장부 편입 확정", use_container_width=True):
                    supabase.table("user_portfolio").insert({
                        "username": username, "ticker": matched_ticker, "name": matched_name, "buy_price": add_p, "qty": add_q
                    }).execute()
                    st.success("포트폴리오 장부 등록에 성공했습니다!")
                    time.sleep(0.5)
                    st.rerun()
        elif user_search_input:
            st.error("❌ 현재 마스터 풀(11개사) 내부에서 매칭되는 종목을 찾지 못했습니다. 정확히 입력해주세요.")
    ...
    with tab_port:
        ...
        if col_sync1.button("🔄 가치 밸류에이션 전면 재연산", width="stretch"):
            ...
            with st.status("v17.5 지능형 데이터 뱅킹 및 리레이팅 연산 중...", expanded=True) as status:
        ...
        if not portfolio_data:
            st.info("장부에 보유 주식이 없습니다. 위의 자산 편입 메뉴에서 추가해주세요.")
            return
        ...
        for row in portfolio_data:
            ...
            krx_sector = raw_trends[1] if (isinstance(raw_trends, list) and len(raw_trends) > 1) else MASTER_STOCK_POOL.get(row['ticker'], {}).get('sector', '일반제조업')
        ...
        def style_mts_color(row):
            ...
            if m_type == "PBR" and "연산 모델" in df_disp.columns: styles[df_disp.columns.get_loc('연산 모델')] = 'background-color: rgba(155, 89, 182, 0.1); color: #9B59B6; font-weight: bold;'
            if peg_val < 1.0 and peg_val > 0 and m_type == "PER" and "진성 PEG" in df_disp.columns: styles[df_disp.columns.get_loc('진성 PEG')] = 'background-color: rgba(0, 180, 100, 0.08); color: #00B464; font-weight: bold;'
        ...
        if selected_indices:
            ...
            # 👍 [v12.0 동선 역산 복구] 그리드에서 체크 시 통제실 헤더에 매수/매도 상호작용 인풋 동시 표출
            st.markdown(f"### 🛠️ [{s_name}] v17.5 퀀트 자산 통제실")
            
            # 여기서 가변 입력창 제어판을 즉시 결합
            with st.container(border=True):
                st.markdown(f"#### ⚙️ [{s_name}] 포트폴리오 가변 거래소 주문창")
                c_trade1, c_trade2, c_trade3 = st.columns(3)
                with c_trade1:
                    mod_p = st.number_input("장부 평단가 수정 (₩)", value=int(raw_row['buy_price']), step=100, key=f"mod_p_{s_ticker}")
                with c_trade2:
                    mod_q = st.number_input("보유 수량 조절 (주)", value=int(raw_row['qty']), step=10, key=f"mod_q_{s_ticker}")
                with c_trade3:
                    st.write("실전 결단")
                    if st.button("✏️ 장부 데이터 변경", use_container_width=True, key=f"btn_mod_{s_ticker}"):
                        supabase.table("user_portfolio").update({"buy_price": mod_p, "qty": mod_q}).eq("id", raw_row['id']).execute()
                        st.success("데이터베이스 원본 정정 완료!")
                        time.sleep(0.3)
                        st.rerun()
                    if st.button("🗑️ 이 포지션 전량 청산 (매도)", type="primary", use_container_width=True, key=f"btn_sell_{s_ticker}"):
                        supabase.table("user_portfolio").delete().eq("id", raw_row['id']).execute()
                        st.error("포지션 전량 매도 완료! 장부에서 제거되었습니다.")
                        time.sleep(0.3)
                        st.rerun()
            ...
            with t1:
                ...
                st.markdown(f"**• TTM 기초 지표:** EPS `{eps_val:,.0f}원` | BPS `{bps_val:,.0f}원` | **진성 PEG:** `{selected_stock['PEG']}x`")
            ...
            with t3:
                if s_cache.get('q_headers') and len(s_cache['q_headers']) >= 2:
                    chart_df = pd.DataFrame({
                        "분기": s_cache['q_headers'], "매출액(억원)": s_cache['q_revenues'], "영업이익(억원)": s_cache['q_op_profits']
                    }).set_index("분기")
    ...
    with tab_hist:
        ...
        if hist_res.data:
            ...
            if 'created_at' in df_hist.columns: df_hist['created_at'] = df_hist['created_at'].apply(format_date_clean)
    ...
    with tab_log:
        ...
        if log_res.data:
            ...
            if 'created_at' in df_log.columns: df_log['created_at'] = df_log['created_at'].apply(format_date_clean)
