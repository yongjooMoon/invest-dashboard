...
def render_exit_risk_content(h, supabase):
    ...
    # 🛡️ XSS 방어: DB에 값이 None이어도 에러나지 않도록 str(or '') 로직 추가
...
def render_detailed_report_content(sel, df_price=None, fund=None, factor_score=None, gates=None):
    ...
    # 🛡️ XSS 방어: DB에 값이 없어서 None이 반환되어도 에러가 나지 않도록 str(or '') 강제 변환
    ...
    safe_region = html.escape(str(sel.get('region') or 'KR'))
        ...
        with c_right:
            ...
            if gates:
                for k, g in gates.items():
                    ...
                    # 🛡️ XSS 방어: 필터명과 사유도 안전하게 이스케이프 처리
                    safe_g_name = html.escape(str(g.get('name') or ''))
                    safe_g_reason = html.escape(str(g.get('reason') or ''))
                    st.markdown(f"**{safe_g_name}** : <span style='color:{color}; font-weight:bold;'>{pass_status}</span> &nbsp;({safe_g_reason})", unsafe_allow_html=True)
            else:
