...
def fetch_naver_fundamentals(raw_code):
    ...
    try:
        ...
        return {"q_headers": q_headers, "q_revenues": q_revenues, "q_op_profits": q_op_profits,
...
# --- [메인 주식 대시보드 스크립트] ---
def run_stock_quant_page(supabase, user_id, naver_id, naver_secret):
    st.title("📈 스마트 프랍 퀀트 포트폴리오 엔진")
    if not naver_id or not naver_secret:
        st.info("⚠️ 상단 '내 API 키 자산 설정' 메뉴에서 네이버 뉴스 API 키쌍을 먼저 등록해 주세요.")
        return

    # KRX 종목 마스터 데이터 수집 (FinanceDataReader 활용 캐싱)
    @st.cache_data
    ...
    with st.expander("➕ 포트폴리오 신규 자산 편입", expanded=False):
        ...
        if st.button("장부 조율 및 매수 결제", type="primary"):
            ...
            # Supabase DB에 유저 고유 관심 주식 행 추가/갱신
            supabase.table("user_portfolio").upsert({
                "user_id": user_id, "ticker": ticker, "name": s_name, "buy_price": buy_p, "qty": qty
            }).execute()
            st.success(f"[{s_name}] 장부에 성공적으로 자산 편입 완료되었습니다.")
            st.rerun()

    # 2. 유저 계정 포트폴리오 로드
    db_res = supabase.table("user_portfolio").select("*").eq("user_id", user_id).execute()
    ...
    # 4. 개별 종목 더블클릭 상세조회 심층 분석 우회 (선택 박스)
    ...
    with t1:
        if target_data["fund"] and len(target_data["fund"]["q_headers"]) >= 2:
            ...
            ax1.bar(target_data["fund"]["q_headers"], target_data["fund"]["q_revenues"], color='#3182F6', alpha=0.8,
                    width=0.3, label="매출액")
            ax2 = ax1.twinx()
            ax2.plot(target_data["fund"]["q_headers"], target_data["fund"]["q_op_profits"], color='#F04452', marker='o',
                     linewidth=2, label="영업이익")
            st.pyplot(fig)
        else:
