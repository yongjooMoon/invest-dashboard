...
def calculate_exit_risk(curr, entry, stop):
    ...
    # 지지선에 도달하면 100%, 진입가(또는 그 이상)이면 0%
    risk = 100 - (distance / buffer * 100)
    return max(0, min(100, int(risk)))

# ══════════════════════════════════════════
# [Component] Exit Risk 팝업 카드 (Dialog)
# ══════════════════════════════════════════
...
# ══════════════════════════════════════════
# [View] Watchlist 팝업 다이얼로그
# ══════════════════════════════════════════
@st.dialog("👀 관심종목 요약 (Watchlist)")
def show_watchlist_dialog(sel):
    col1, col2 = st.columns([1, 1])
    with col1:
        st.markdown(f"**종목:** {sel['name']} ({sel['symbol']})")
        st.markdown(f"**현재가:** {sel['current_price']:,}원")
    with col2:
        st.markdown(f"**랭킹 스코어:** <span style='color:#00B464;'>{sel.get('factor_score', 0):.2f}점</span>", unsafe_allow_html=True)
        st.markdown(f"**모멘텀 강도:** <span style='color:#F04452'>{sel.get('momentum_score', 0):+.2f}%</span>", unsafe_allow_html=True)

    st.caption(f"💡 추격매수 절대 조건 {sel.get('total_pass', 0)}/6 통과 중")

    if st.button("상세 리포트 보기 ➔", use_container_width=True, type="primary"):
...
def render_stock_search_view():
    ...
    if selected_stock_str:
        ...
        sel = {
            'symbol': search_query, 'name': selected_stock_str.split(" (")[0], 'current_price': df['Close'].iloc[-1],
...
def run_stock_quant_page(supabase, username: str = "admin", **kwargs):
    ...
    if st.session_state.current_view == "main":
        ...
        with tab_port:
            ...
            st.caption("💡 표에서 종목을 클릭하시면 Exit Risk 상세 분석 팝업이 나타납니다.")
            if holdings:
                h_data = []
                for h in holdings:
                    curr = h.get("current_price", 0)
                ...
                # 포트폴리오 테이블: 클릭 시 팝업을 띄우기 위해 on_select 복구
                sel_h = st.dataframe(
                    df_h[["Stock", "Entry Price", "Current", "P&L", "Stop", "Recent 30d", "Exit Risk"]].style.map(
                    ...
                    hide_index=True, use_container_width=True, on_select="rerun", selection_mode="single-row", key="port_table"
                )

                if sel_h and hasattr(sel_h, "selection") and sel_h.selection.rows:
                    show_exit_risk_dialog(h_data[sel_h.selection.rows[0]]["RawData"])
            else:
        ...
        with tab_watch:
            ...
            st.caption("💡 그리드에서 종목을 클릭하시면 요약 팝업 및 상세 리포트를 확인하실 수 있습니다.")
            ...
            if confirmed:
                ...
                sel_c = st.dataframe(df_c.drop(columns=["RawData"]), use_container_width=True, on_select="rerun", selection_mode="single-row", hide_index=True, key="conf_grid")
                if sel_c and hasattr(sel_c, "selection") and sel_c.selection.rows:
                    show_watchlist_dialog(df_c.iloc[sel_c.selection.rows[0]]["RawData"])
                st.divider()

            if watchlist:
                st.caption(f"👀 추격매수 예비 돌파 종목 ({len(watchlist)}개) - 4조건 이상 통과")
                df_w = build_w_table(watchlist[:15])
                sel_w = st.dataframe(df_w.drop(columns=["RawData"]), use_container_width=True, on_select="rerun", selection_mode="single-row", hide_index=True, key="watch_grid")
                if sel_w and hasattr(sel_w, "selection") and sel_w.selection.rows:
                    show_watchlist_dialog(df_w.iloc[sel_w.selection.rows[0]]["RawData"])
            else:
